...
class Encoder(layers.Layer):
    ...
    def __init__(self, conv_depth, h_layers=0, name="encoder",  **kwargs):
        ...
        for depth in range(h_layers):
            self.sublayers.append(
                tfc.SignalConv2D(
                    num_filters,
                    (5, 5),
                    name="layer_{}".format(depth+2),
                    corr=True,
                    strides_down=1,
                    padding="same_zeros",
                    use_bias=True,
                    activation=tfc.GDN(name="gdn_{}".format(depth+2)),
                )
            )
            self.sublayers.append(
                layers.PReLU(shared_axes=[1, 2])
...
class Decoder(layers.Layer):
    ...
    def __init__(self, n_channels, h_layers, name="decoder", **kwargs):
        ...
        for depth in range(h_layers):
            ...
            tfc.SignalConv2D(
                num_filters,
                (5, 5),
                name="layer_{}".format(depth),
                corr=False,
                strides_up=1,
                padding="same_zeros",
                use_bias=True,
                activation=tfc.GDN(name="igdn_{}".format(depth), inverse=True),
                )
            )
            self.sublayers.append(
                layers.PReLU(shared_axes=[1, 2])
        ...
        self.sublayers.append(
            tfc.SignalConv2D(
                num_filters,
                (5, 5),
                name="layer_{}".format(h_layers),
                corr=False,
                strides_up=2,
                padding="same_zeros",
                use_bias=True,
                activation=tfc.GDN(name="igdn_{}".format(h_layers), inverse=True),
            )
        )
        self.sublayers.append(
            layers.PReLU(shared_axes=[1, 2])
...
class DeepJSCCF(layers.Layer):
    def __init__(
        ...
        **kwargs
        ...
        self.layer = layer_id
        self.encoder = Encoder(conv_depth, h_layers)
        self.decoder = Decoder(n_channels, h_layers, name="decoder_output")
...
def main(args):
    ...
    # parameter tuning with kerastuner
    for layer in range(args.n_layers):
        ...
        def model_builder(hp):
            # h_layers tuning
            hp_h_layers = hp.Int('h_layers', min_value = 0, max_value = 0, step = 1)
            ae_layer = DeepJSCCF(
            channel_snr,
            int(args.conv_depth),
            args.channel,
        ...
        tuner = kt.Hyperband(model_builder,
                            objective = kt.Objective('psnr_metric',direction='max'),
                            max_epochs = 3,
                            factor = 3,
        ...
        print(f"""
        # The hyperparameter search is complete. The optimal number of hidden layer is 
        # {best_hps.get('h_layers')}.
        """)
        ...
        if not (args.run_eval_once or (layer == 0 and args.pretrained_base_layer)):
            ...
            history = model.fit(
                ...
                validation_steps=(
                    DATASETS[args.dataset_train]._NUM_IMAGES["validation"] // args.batch_size_train
                ),
            )

            # save train history
            history_dict = history.history
            print("#Save history plot#")
            epochs = range(1, len(history_dict['psnr_metric']) + 1)
            plt.plot(epochs, history_dict['psnr_metric'], label='psnr_metric')
            plt.title('PSNR')
            plt.xlabel('Epochs')
            plt.ylabel('psnr')
            plt.legend()
            plt.savefig("h={}n={}.png".format(args.h_layers, layer))
            with open("h={}n={}".format(args.h_layers, layer), 'wb') as file:
    ...
    print("# Save final plot #")
    plt.savefig("h_layers={} result.png".format(args.h_layers))
    ...
    for eval_model in models:
        ...
        for m, v in zip(eval_model.metrics_names, out_eval):
            ...
            if met_name == "eval_psnr_metric":
                psnr_per_depth.append(v)
            print("{}={}".format(met_name, v), end=" ")

    for psnr in psnr_per_depth:
        print("eval_psnr_metric=", psnr)
...
if __name__ == "__main__":
    ...
    p.add(
        ...
        type=int,
        default=3,
        help=("Number of hidden layers/layers used in the transmission"),
        required=True,
    )
